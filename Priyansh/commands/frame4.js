...
module.exports.run = async function ({ event, api, args }) {    
  ...
  if (!mention[0]) return api.sendMessage("Please mention 1 person.", threadID, messageID);
      ...
      return makeImage({ one, two }).then(path => api.sendMessage({ body: "★᭄𝗖𝗿𝗲𝗱𝗶𝘁𝘀  *🌺 𝗢𝗪𝗡𝗘𝗥 👑➪🦋⃟⃟ ⍣⃝ 𝑵𝑲➺𝑬𝑫𝑰𝑻𝑶𝑹༆𓆪⃟⍨⃝🖤", attachment: fs.createReadStream(path) }, threadID, () => fs.unlinkSync(path), messageID));
  }
}
